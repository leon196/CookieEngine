...
void main () {
	...
	float angle = segment * growAngle + sequence;
	float radius = .5 + segment * growRadius;
	float height = 1. + segment * growHeight + sin(segment*growWave-time+sequence*growWaveOffset)*growWaveScale;
