...
void main ()
{
	...
	//get the difference
	float scr_dif = texture2D(sceneTexture, uv).r - texture2D(lastFrameTexture, uv).r;

	//calculate the gradient
	vec3 gradx; vec3 grady; vec3 gradmag; vec3 vx; vec3 vy;
	gradx = texture2D(lastFrameTexture, uv + off_x).rgb - texture2D(lastFrameTexture, uv - off_x).rgb;
	gradx += texture2D(sceneTexture, uv + off_x).rgb - texture2D(sceneTexture, uv - off_x).rgb;
	grady = texture2D(lastFrameTexture, uv + off_y).rgb - texture2D(lastFrameTexture, uv - off_y).rgb;
	grady += texture2D(sceneTexture, uv + off_y).rgb - texture2D(sceneTexture, uv - off_y).rgb;
